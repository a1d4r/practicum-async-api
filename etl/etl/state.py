...
from etl.storage.base import BaseStorage

T = TypeVar("T", bound=BaseModel)


class ETLState(BaseModel):
    """Состояние ETL-процесса."""

    persons_modified_cursor: datetime
    genres_modified_cursor: datetime
    film_works_modified_cursor: datetime
...
class StateManager(Generic[T]):
    ...
    def _get_state(self) -> T:
        ...
        try:
            ...
            logger.warning(
                "State data is invalid. Falling back to default state: {}",
                self.default_state,
            )
